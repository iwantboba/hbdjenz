document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateInitialCandles() {
    ...
    // Get icing dimensions relative to the cake container
    const icingLeft = icing.offsetLeft;
    const icingTop = icing.offsetTop;
    const icingWidth = icing.offsetWidth;
    const icingHeight = icing.offsetHeight;

    const centerX = icingLeft + icingWidth / 2;
    const centerY = icingTop + icingHeight / 2;
    // Subtract a little to keep candles from the very edge
    const radiusX = icingWidth / 2 - 10;
    const radiusY = icingHeight / 2 - 10;

    for (let i = 0; i < candleCount; i++) {
      const angle = Math.random() * 2 * Math.PI;
      // Use sqrt of random for a uniform distribution over the area
      const radius = Math.sqrt(Math.random());

      const x = centerX + radiusX * radius * Math.cos(angle);
      const y = centerY + radiusY * radius * Math.sin(angle);

      // Adjust for candle width (12px) to center it on the point
      addCandle(x - 6, y);
    }
  }
